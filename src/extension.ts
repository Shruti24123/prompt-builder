...
type WatcherInfo = { watcher: vscode.FileSystemWatcher; refs: number };

let tagDecoInline: vscode.TextEditorDecorationType;
let tagDecoHide: vscode.TextEditorDecorationType;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration(e => {
      if (!e.affectsConfiguration('tagHighlighter')) return;
      disposeDecorations();
      createDecorations();
      debounced();
    }),
    vscode.window.onDidChangeActiveTextEditor(ed => ed && void updateDecorations(ed)),
    vscode.workspace.onDidChangeTextDocument(e => {
      const ed = vscode.window.activeTextEditor;
      if (ed && e.document === ed.document) debounced();
    }),
    ...
    vscode.commands.registerCommand('tagHighlighter.openSubstitutedPreview', () =>
      openVirtualPreview()
    )
  );
...
function createDecorations() {
  ...
  tagDecoInline = vscode.window.createTextEditorDecorationType({
    after: {
      color: cfg.get<string>('inlineColor') || '#888',
      margin: '0 0 0 6px',
...
async function updateDecorations(editor: vscode.TextEditor) {
  ...
  const cfg = vscode.workspace.getConfiguration('tagHighlighter');
  const pattern = safeRegex(cfg.get<string>('pattern') || '<<[A-Za-z0-9._\\-/:\\\\]+?>>');
  const sep = cfg.get<string>('pathSeparator') || ':';
  const onlyExisting = cfg.get<boolean>('onlyHighlightExistingPaths') ?? true;
  const showInline = cfg.get<boolean>('showInline') ?? true;
  const showHover = cfg.get<boolean>('showHover') ?? true;
  const maxBytes = Math.max(0, cfg.get<number>('maxBytes') ?? 8192);
  const joiner = cfg.get<string>('newlineJoiner') || ' ↩ ';
  const baseDir = path.dirname(editor.document.uri.fsPath);

  const text = editor.document.getText();
  const matches: { range: vscode.Range; inner: string; absPath: string }[] = [];
  ...
  const hideDecos: vscode.DecorationOptions[] = [];
    ...
    matches.map(async (it) => {
      ...
      if (exists) {
        ...
        retainWatcher(it.absPath, () => {
          ...
          const ed = vscode.window.activeTextEditor;
          if (ed) void updateDecorations(ed);
        });
      }
      ...
      const hover = showHover
        ...
        : undefined;
      if (hover) hover.isTrusted = false;
  ...
  for (const [p, info] of WATCHERS) {
    if (!needed.has(p)) releaseWatcher(p);
  }
}
...
function safeRegex(src: string): RegExp {
  try { return new RegExp(src, 'g'); } catch { return /<<[A-Za-z0-9._\-/:\\]+?>>/g; }
}

function toAbsolute(baseDir: string, tagInner: string, sep: string): string {
...
async function readFileTruncated(p: string, maxBytes: number): Promise<string> {
  ...
  try {
    ...
    if (maxBytes && buf.byteLength > maxBytes) buf = buf.subarray(0, maxBytes);
...
function retainWatcher(absPath: string, onAnyChange: () => void) {
...
function releaseWatcher(absPath: string) {
...
const _onDidChange = new vscode.EventEmitter<vscode.Uri>();
